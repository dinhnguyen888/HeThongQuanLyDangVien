    ...
    public class ChuyenSinhHoatDangService
    {
        ...
        public List<ChuyenSinhHoatDangDTO> GetByDangVienID(int dangVienID)
        {
            using (var conn = DbHelper.GetConnection())
            {
                var sql = @"
                    SELECT 
                        cshd.ChuyenSinhHoatID, cshd.DangVienID, dv.HoTen AS HoTenDangVien,
                        cshd.DonViDi, ddi.TenDonVi AS TenDonViDi,
                        cshd.DonViDen, dden.TenDonVi AS TenDonViDen,
                        cshd.NgayChuyen, cshd.LyDo, cshd.GhiChu, cshd.FileQuyetDinh,
                        cshd.TrangThai, cshd.NgayTao, cshd.NguoiTao
                ...
                var result = conn.Query<ChuyenSinhHoatDangDTO>(sql, parameters).ToList();
                return result;
            }
        }
        ...
        public (int id, string error) Insert(ChuyenSinhHoatDang chuyenSinhHoat)
        {
            using (var conn = DbHelper.GetConnection())
            {
                ...
                parameters.Add("@NguoiTao", chuyenSinhHoat.NguoiTao);

                try
                {
                    var result = conn.QuerySingle<int>(sql, parameters);
                    return (result, null);
                }
                catch (Exception ex)
                {
